...
function LoginPage({ goToSignup, goToHome, setProfile }) {
  ...
  // No intermediate message: immediately navigate to Sign Up on relevant failures
  ...
  // nothing to reset
    ...
    try {
      ...
      if (response.ok) {
        ...
        // If server indicates user not found, immediately navigate to Sign Up
        const msg = result.message || 'Login failed';
        if (response.status === 404 || /user not found|no user|not registered/i.test(msg)) {
          goToSignup();
          return;
        }
        // For other server errors, also navigate to Sign Up as requested
        goToSignup();
        return;
      }
    } catch (error) {
      console.error("Login error:", error);
      // Network or CORS error — immediately go to Sign Up as requested
      goToSignup();
    }
  };
